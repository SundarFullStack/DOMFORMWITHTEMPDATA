...
function data_mapping() {
    ...
    let element = [Chinese_input,nif_input,sif_input,Japanese_input,American_input]


    for (let i = 0; i < element.length; i++){

        let checkbox = element[i] 

        if (checkbox.checked) {
            
            trd6.innerText = checkbox.value;
        }
    }
    ...
    let table_header = create_element('h4', 'class', 'col-9 text-center mt-5');
...
let nif_input = create_input_check('input', 'class', 'form-check-input', 'value', 'North_Indian', 'type', 'checkbox');
...
let sif_input = create_input_check('input', 'class', 'form-check-input', 'value', 'North_Indian', 'type', 'checkbox');
...
let submit = create_label('button', 'class', 'btn btn-success btn-md mb-5', 'Submit');
